...
extern "C" {
    ...
    void drawGFButton_(const char* tag, int x, int y, int w, int h);
    ...
    void addNoTitleIconView_(int x, int y, int w, int h);
...
void drawGFButton_(const char* tag, int x, int y, int w, int h) {
    // NSLog(@"#drawGFButton_");
    UIViewController* parent = getUnityViewController();
    NSString* stringTag = charToString(tag);
    
    GFWallButton *gfButton = [[[GFWallButton alloc] init:parent] autorelease];
    gfButton.frame = CGRectMake(x, y, w, h);
    [[[GFWallButtonController alloc]init:gfButton gfButtonTag:stringTag] autorelease];
    
    [parent.view addSubview:gfButton];
...
// GFIconController.addNoTitleIconView
void addNoTitleIconView_(int x, int y, int w, int h) {
    UIViewController* parent = getUnityViewController();
