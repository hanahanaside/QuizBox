 ...
 */
	...
	[DllImport("__Internal")]
    private static extern void showTriggerImageAd_(string triggerId, int locateX, int locateY, bool onStatusArea, string orientation, string appKey);
	public static void showTriggerImageAd(string triggerId = "", int locateX = 0, int locateY = 0, bool onStatusArea = false, string orientation = "", string appKey = "") {
		if (Application.platform != RuntimePlatform.OSXEditor) {
    		showTriggerImageAd_(triggerId, locateX, locateY, onStatusArea, orientation, appKey);
		}
	}
